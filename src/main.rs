...
pub extern fn main() -> () {
    ...
    if let Some(ref mut w5500) = w5500 {
        ...
        if let Some(ref socket) = udp_server_socket {
            ...
            //     (&mut w5500, socket).blocking_send(&ip, port, &response_buffer[..response_length]).unwrap();
            // }
        }
    }
}
